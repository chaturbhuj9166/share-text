...
export default function Chat() {
  ...
  useEffect(() => {
    const loadMessages = async () => {
      ...
      /**
       * ✅ PRIORITY:
       * 1️⃣ Header se aaya hua user (best)
       * 2️⃣ Messages se fallback (direct URL case)
       */
      if (location.state?.otherUser) {
        setOtherUser(location.state.otherUser);
      } else if (res.data.length > 0) {
        // jis sender ki id meri nahi hai → wahi dusra user
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-80px)] bg-slate-100">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-2">
        {messages.map((msg) => {
          // ⚠️ abhi auth id nahi hai, later fix hoga
          const isMe = false;
